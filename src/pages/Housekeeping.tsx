...
import { Plus, Search, Sparkles, Clock, CheckCircle, Loader2, Home } from 'lucide-react';
...
export default function Housekeeping() {
  ...
  const { data: services = [], isLoading } = useQuery<Service[]>({
    queryKey: ['/api/services', 'housekeeping'],
    queryFn: async () => {
      return apiRequest('/services?type=housekeeping');
    }
  });
  ...
  const createMutation = useMutation({
    mutationFn: async (data: typeof formData) => {
      return apiRequest('/services', {
        method: 'POST',
        body: JSON.stringify({ ...data, type: 'housekeeping' })
      });
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['/api/services'] });
      toast({ title: 'Housekeeping request created successfully' });
      setIsAddOpen(false);
      resetForm();
    },
    onError: (error: Error) => {
    ...
    <div className="p-6 space-y-6">
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-muted-foreground">Manage room cleaning and housekeeping requests</p>
        </div>
        <Dialog open={isAddOpen} onOpenChange={setIsAddOpen}>
          <DialogTrigger asChild>
            <Button data-testid="button-new-housekeeping">
          ...
          <DialogContent>
            <DialogHeader>
              <DialogTitle>New Housekeeping Request</DialogTitle>
            </DialogHeader>
            <div className="space-y-4">
              <div>
                ...
                <Input
                  ...
                  placeholder="e.g., Room Cleaning, Deep Clean"
              ...
              <Button onClick={handleSubmit} className="w-full" disabled={createMutation.isPending} data-testid="button-submit-housekeeping">
            ...
            <div className="flex gap-2">
              <div className="relative">
                ...
                <Input
                  ...
                  className="pl-9 w-48"
                  data-testid="input-search-housekeeping"
                />
              </div>
        ...
        <CardContent>
          {filteredServices.length === 0 ? (
            <div className="text-center py-12 text-muted-foreground">
              ...
              <p>No housekeeping requests found</p>
            </div>
          ) : (
              ...
              <table className="w-full">
                <thead>
                  <tr className="border-b">
                    ...
                    <th className="text-left py-3 px-4 font-medium">Tenant / Room</th>
                ...
                <tbody>
                  {filteredServices.map((service: Service) => (
                    <tr key={service._id} className="border-b hover:bg-muted/50" data-testid={`row-housekeeping-${service._id}`}>
                      ...
                      <td className="py-3 px-4">
                        <div className="flex items-center gap-2">
                          ...
                          <div>
                            <p>{service.tenantId?.name || 'N/A'}</p>
                            <p className="text-sm text-muted-foreground">Room: {service.tenantId?.roomNumber || 'N/A'}</p>
                          </div>
                        </div>
                      </td>
